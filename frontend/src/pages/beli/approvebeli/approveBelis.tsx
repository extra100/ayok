...
const ApproveBelis: React.FC = () => {
  ...
  const calculation = (posId: string, selectedJenisPajak: string) => {
    ...
    if (currentFields) {
      const { qty_solder, harga_jual, diskon } = currentFields

      const total =
        parseInt(qty_solder) * parseInt(harga_jual) - parseInt(diskon)
                    ...
                    >
                      <Input.Group compact>
                        ...
                        <div className="my-select-container">
                          ...
                          >
                            {coas
                              ?.filter((e: any) => e.kategori === 'Kas & Bank')
                              .map((e: any) => (
